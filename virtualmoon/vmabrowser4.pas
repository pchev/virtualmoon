...
uses
  ...
  passql, passqlite,skylib, dbutil, ComCtrls, ExtCtrls, IniFiles;
...
if Mlb.LoadFromFile(edit1.text) then begin
  ...
  Label5.Caption:=inttostr(Mlb.RowCount)+' '+form1.m[8];
...
begin
StringGrid1.Cells[0,0]:=form1.m[9];
StringGrid1.Cells[1,0]:=form1.m[10];
dbu.DataBase:=dbname;
edit1.Text:='';
end;
...
if (i<1) or (not Checklistbox1.Checked[i-1]) then begin showmessage(form1.m[11]);exit; end;
fieldmode[i]:=1;
...
begin
i:=Checklistbox1.ItemIndex+1;
if i<1 then begin showmessage(form1.m[11]);exit; end;
j:=StringGrid1.Selection.Top;
if j<1 then begin showmessage(form1.m[12]);exit; end;
fieldmode[i]:=2;
fieldlist[i]:=j;
CheckListBox1Click(Sender);
end;
...
case fieldmode[i] of
 ...
 1: begin
    msg.Text:=form1.m[14]+': '+fieldconst[i];
    ConstantText.Text:=fieldconst[i];
    end;
 2: begin
    msg.Text:=form1.m[15]+': '+StringGrid1.Cells[0,fieldlist[i]];
    ConstantText.Text:='';
    end;
end;
memo1.Clear;
...
begin
memo1.Clear;
memo1.Lines.Add(form1.m[16]);
for i:=1 to maxcol do begin
   if checklistbox1.Checked[i-1] and (fieldmode[i]=0) then errormsg(checklistbox1.Items[i-1]+' '+form1.m[17]);
   if not checklistbox1.Checked[i-1] then fieldmode[i]:=0;
end;
memo1.Lines.Add(form1.m[18]);
if fieldmode[1]=0 then errormsg(form1.m[19]+' '+checklistbox1.Items[0]);
if fieldmode[1]<>1 then errormsg(form1.m[1]);
if fieldmode[2]=0 then errormsg(form1.m[19]+' '+checklistbox1.Items[1]);
if fieldmode[20]=0 then errormsg(form1.m[19]+' '+checklistbox1.Items[19]);
if fieldmode[22]=0 then errormsg(form1.m[19]+' '+checklistbox1.Items[21]);
memo1.Lines.Add(form1.m[20]);
// insert to database
mlb.GoFirst;
memo1.Lines.Add(form1.m[21]+' '+dbname);
...
repeat
  ...
  if not dbu.Query(cmd) then raise exception.Create(form1.m[24]+crlf+dbu.GetErrorMessage+crlf+cmd);
  mlb.GoNext;
until mlb.EndOfFile;
...
dbu.Query(cmd);
dbu.Commit;
memo1.Lines.Add(form1.m[22]);
memo1.Lines.Add(form1.m[23]);
Form1.dbselection:='DBN in ('+dbn+')';
except
  dbu.Rollback;
  memo1.Lines.Add(form1.m[24]);
  memo1.Lines.Add(form1.m[25]+' '+form1.m[26]);
  raise
end;
end;
