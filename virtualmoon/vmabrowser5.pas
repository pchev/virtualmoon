...
type
  ...
  public
    { Public declarations }
    dblist: TStringList;
    dbn1,dbn2,dbn3,dbn4,dbn5: string;
  end;
...
begin
lst:='';
i:=pos('(',Form1.dbselection);
if i>0 then begin
   lst:=copy(Form1.dbselection,i,9999);
...
if Form1.dbm.Query('select DBN,NAME from user_database') then begin
  for i:=0 to Form1.dbm.RowCount-1 do begin
    buf:=Form1.dbm.Results[i][0];
    dblist.Add(buf);
    CheckListBox1.Items.Add(Form1.dbm.Results[i][1]);
    inc(n);
