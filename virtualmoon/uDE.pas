...
function load_de_file(jd: double; de_folder: PChar; de_type: integer): boolean;
...
begin
    ...
    result := false;
    djd(jd,de_year,m,d,hour);
   	//using the year to find the file and name it.
   	Case de_type of
    	200:    begin  //1600 to 2200 - 50 years steps
           		    if InRange(de_year, 1600, 2200) then begin
           		        de_year := 1600 + floor((de_year - 1600) / 50) * 50;
           		        de_file := 'unxp' + floatToStr(de_year) + '.200';
                    end else exit;
         	    end;
   	    403:    begin  //1950 to 2025 - 25 years step
           	        if InRange(de_year, 1950, 2025) then begin
           		        de_year := 1950 + floor((de_year - 1950) / 25) * 25;
           		        de_file := 'unxp' + floatToStr(de_year) + '.403';
                    end else exit;
         	    end;
   	    405:    begin  //1600 to 2200 - 50 years step
           		    if InRange(de_year, 1600, 2200) then begin
           		        de_year := 1600 + floor((de_year - 1600) / 50) * 50;
           		        de_file := 'unxp' + floatToStr(de_year) + '.405';
                    end else exit;
         	    end;
            406:    begin  // -3000 to 3000 - 300 years step
           		    if InRange(de_year, -3000, 3000) then begin
           		        de_year := -3000 + floor((de_year - (-3000)) / 300) * 300;
                            if de_year < 0 then
                    	    de_file := 'unxm' + FormatFloat('0000',abs(de_year)) + '.406'
           		        else
                    	    de_file := 'unxp' + FormatFloat('0000',de_year) + '.406';
                    end else exit;
                    end;
   	    421:    begin  //1900 to 2050 - 1 file
           	        if InRange(de_year, 1900, 2050) then begin
           		        de_year := 1900;
           		        de_file := 'unxp' + floatToStr(de_year) + '.421';
                    end else exit;
         	    end;
   	end;
