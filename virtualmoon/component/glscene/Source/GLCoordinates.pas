...
uses Classes, VectorGeometry, VectorTypes, OpenGLTokens, BaseClasses,
  GLCrossPlatform;
...
begin
  Assert(FStyle = CsPoint2D, CsPoint2DHelp);
  VectorGeometry.MakeVector(FCoords, Vector);
  NotifyChange(Self);
end;
...
begin
  Assert(FStyle = CsPoint2D, CsPoint2DHelp);
  VectorGeometry.MakeVector(FCoords, Vector.V[0], Vector.V[1], 0);
  NotifyChange(Self);
end;
