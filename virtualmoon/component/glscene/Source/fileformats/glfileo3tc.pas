...
begin
  ...
  if ChunkHeader.NumMipmaps<>0 then fMipLevels := ChunkHeader.NumMipmaps
  else fMipLevels := 1;
  ...
  case ChunkHeader.InternalPixelFormat of
    ...
    else raise EInvalidRasterFile.Create('Unsupported O3TC format.')
  end;
...
begin
  ...
  begin
    ...
    Header.Version := 2;
    if fCubeMap then ChunkHeader.Extension := ChunkHeader.Extension or O3_TC_CUBE_MAP;
    if fTextureArray then ChunkHeader.Extension := ChunkHeader.Extension or O3_TC_ARRAY;
  end;
  ...
  try
    ...
    begin
      ...
      repeat
        ...
        glGetTexLevelParameteriv(textureTarget, fMipLevels, GL_TEXTURE_INTERNAL_FORMAT,
          @texFormat);
        if texFormat = 1 then Break;
        Inc(fMipLevels);
        ...
        begin
          ...
          if texLod>optLod then texLod:=optLod;
          // Check for MipMap posibility
          if ((fInternalFormat>=tfFLOAT_R16)
          and (fInternalFormat<=tfFLOAT_RGBA32)) then texLod := 1;
        end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if GL_NV_texture_compression_vtc and (d>0) and not fTextureArray then
              begin
                if level=0 then GetMem( vtcBuffer, LevelSize(0));
              ...
              else glGetCompressedTexImage(textureTarget, level, lData);
            end
            else
              glGetTexImage(textureTarget, level, fColorFormat, fDataType, lData);

            Inc( lData, LevelSize(level));
          end;  // for level
        end; // for face
        if Assigned(vtcBuffer) then FreeMem(vtcBuffer);
