...
uses vmabrowser1, vmabrowser5, skylib;
...
if not allchecked then begin
 for i:=0 to CheckListBox1.Count-1 do begin
  if CheckListBox1.Checked[i] then begin
     if i>=22 then wildcard:='%';
     if first then begin
        buf:=' and ( (TYPE LIKE "'+Form1.dbtype[i+1]+wildcard+'")';
        first:=false;
     end else begin
        buf:=buf+ ' or (TYPE LIKE "'+Form1.dbtype[i+1]+wildcard+'")';
     end;
  end;
 end; // for
...
sel.Text:=Form1.dbselection+buf;
end;
...
case RadioGroup2.ItemIndex of
  0 : begin
        ...
        if trim(coleq.Text)<>'' then
           sel.Text:=Form1.dbselection+' and ('+fieldlist2.Text+' LIKE "'+coleq.Text+'")';
      end;
  1 : begin
        coleq.Enabled:=false;
        colgt.Enabled:=true;
        ...
        if trim(colgt.Text)<>'' then
           sel.Text:=Form1.dbselection+' and ('+fieldlist2.Text+' >= "'+colgt.Text+'") ORDER BY '+fieldlist2.Text+' ASC';
      end;
  2 : begin
        coleq.Enabled:=false;
        colgt.Enabled:=false;
        ...
        if trim(collt.Text)<>'' then
           sel.Text:=Form1.dbselection+' and ('+fieldlist2.Text+' <= "'+collt.Text+'") ORDER BY '+fieldlist2.Text+' DESC';
      end;
  3 : begin
        coleq.Enabled:=false;
        colgt.Enabled:=false;
        ...
        if (trim(colbetween1.Text)<>'')and(trim(colbetween2.Text)<>'') then
           sel.Text:=Form1.dbselection+' and ('+fieldlist2.Text+' BETWEEN "'+colbetween1.Text+'" AND "'+colbetween2.Text+'") ORDER BY '+fieldlist2.Text+' ASC';
      end;
end;
...
if SelectDB.modalresult=mrOk then begin
   Form1.dbselection:='DBN in (';
   n:=0;
   for i:=0 to SelectDB.CheckListBox1.Count-1 do begin
     if SelectDB.CheckListBox1.Checked[i] then begin
        if n>0 then Form1.dbselection:=Form1.dbselection+',';
        Form1.dbselection:=Form1.dbselection+SelectDB.dblist[i];
        inc(n);
     end;
   end;
   Form1.dbselection:=Form1.dbselection+')';
   CheckListBox1ClickCheck(Sender);
end;
end;
