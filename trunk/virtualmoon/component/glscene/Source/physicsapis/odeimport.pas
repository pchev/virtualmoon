...
enum {
  ...
  TdJointTypeNumbers =
    ...
    dJointTypeSlider,
    dJointTypeContact,
    {$WARNING crossbuilder - check why this is different }
    {$IFDEF FPC}
    dJointTypeUniversal,
//    dJointTypeHinge2,
    {$ELSE}
...
type
  TdSurfaceParameters = packed record
    // must always be defined
    mode : cardinal;
    mu : TdReal;
...
struct dxHashSpace : public dxSpace {
  ...
  TdxGeom = packed record // a dGeomID is a pointer to this
    _class : PdxGeomClass;	// class of this object

    {$ifdef cSINGLE}
    Padding : array[0..1] of integer;
    {$else}
    Padding : array [0..1] of integer;
    {$endif}
...
//  procedure dJointCorrectHinge2(const dJointID : TdJointID); cdecl; external {$IFDEF __GPC__}name 'dJointCorrectHinge2'{$ELSE} ODEDLL{$ENDIF __GPC__};
  ...
  procedure dGeomCCylinderGetParams(const Geom : PdxGeom; var radius, length: TdReal); cdecl; external {$IFDEF __GPC__} name 'dGeomCCylinderGetParams'{$ELSE} ODEDLL name {$IFDEF WINDOWS} 'dGeomCCylinderGetParams' {$ELSE} 'dGeomCapsuleGetParams' {$ENDIF}{$ENDIF __GPC__};
  procedure dGeomCCylinderSetParams(const Geom : PdxGeom; const radius, length: TdReal); cdecl; external {$IFDEF __GPC__} name 'dGeomCCylinderSetParams'{$ELSE} ODEDLL name {$IFDEF WINDOWS} 'dGeomCCylinderSetParams' {$ELSE} 'dGeomCapsuleSetParams' {$ENDIF}{$ENDIF __GPC__};
  ...
  function dGeomCCylinderPointDepth (const Geom : PdxGeom; const x,y,z : TdReal) : TdReal; cdecl; external {$IFDEF __GPC__} name 'dGeomCCylinderPointDepth'{$ELSE} ODEDLL name {$IFDEF WINDOWS} 'dGeomCCylinderPointDepth' {$ELSE} 'dGeomCapsulePointDepth' {$ENDIF}{$ENDIF __GPC__};
...
//  function dCreateTerrainZ(const Space: PdxSpace; pHeights: PdRealHugeArray; vLength: TdReal; nNumNodesPerSide: Integer; bFinite, bPlaceable: Integer): PdxGeom; cdecl;
  ...
  function EXT_dCreateCCylinder(const Space : PdxSpace; const radius, length: TdReal): PdxGeom; cdecl; external ODEDLL name {$IFDEF WINDOWS} 'dCreateCCylinder' {$ELSE} 'dCreateCapsule' {$ENDIF};
...
//  function dGeomTerrainZPointDepth(const Geom: PdxGeom; const x, y, z: TdReal): TdReal; cdecl; external {$IFDEF __GPC__}name 'dGeomTerrainZPointDepth'{$ELSE} ODEDLL{$ENDIF __GPC__};}
  ...
  //k00m
  procedure dGeomTriMeshDataPreprocess(g: PdxTriMeshData); cdecl; external {$IFDEF __GPC__}name 'dGeomTriMeshDataPreprocess'{$ELSE} ODEDLL{$ENDIF __GPC__};
  procedure dGeomTriMeshDataGetBuffer(g: PdxTriMeshData; buf: Pointer; bufLen: Integer); cdecl; external {$IFDEF __GPC__}name 'dGeomTriMeshDataGetBuffer'{$ELSE} ODEDLL{$ENDIF __GPC__};
  procedure dGeomTriMeshDataSetBuffer(g: PdxTriMeshData; buf: Pointer); cdecl; external {$IFDEF __GPC__}name 'dGeomTriMeshDataSetBuffer'{$ELSE} ODEDLL{$ENDIF __GPC__};
  //
  ...
  procedure dMassSetCappedCylinder(var m: TdMass; density: TdReal; direction: Integer; radius, length: TdReal); cdecl; external {$IFDEF __GPC__} name 'dMassSetCappedCylinder'{$ELSE} ODEDLL name {$IFDEF WINDOWS} 'dMassSetCappedCylinder' {$ELSE} 'dMassSetCapsule' {$ENDIF} {$ENDIF __GPC__};
  procedure dMassSetCappedCylinderTotal(var m: TdMass; total_mass: TdReal; direction: Integer; radius, length: TdReal); cdecl; external {$IFDEF __GPC__} name 'dMassSetCappedCylinderTotal'{$ELSE} ODEDLL name {$IFDEF WINDOWS} 'dMassSetCappedCylinderTotal' {$ELSE} 'dMassSetCapsuleTotal' {$ENDIF}{$ENDIF __GPC__};
...
begin
  ...
  VBody := Body^;
  Assert(VBody.mass.mass = m.mass, 'Body test 3 fails');
  ...
  VGeom := Geom^;
  Assert(dGeomGetBody(Geom)=VGeom.Body, 'Geom test 2 fails');
