...
type
  ...
  published
    ...
    {: Amalgamate the dummy's children in a single OpenGL entity.<p>
       ...
       number of frames. }
    property Amalgamate: Boolean read FAmalgamate write SetAmalgamate default
      False;
  ...
  published
    ...
    property FadeTresholdSize: Single read FFadeTresholdSize write
      SetFadeTresholdSize stored False;
    {: Components XYZ are for constant, linear and quadratic attenuation. }
    property DistanceAttenuation: TGLCoordinates read FDistanceAttenuation write
  ...
  published
    ...
    property PointParameters: TGLPointParameters read FPointParameters write
      SetPointParameters;
  ...
  published
    ...
    property AntiAliased: Boolean read FAntiAliased write SetAntiAliased default
      False;
    ...
    property LineWidth: Single read FLineWidth write SetLineWidth stored
      StoreLineWidth;
    property Visible;
  end;
  ...
  protected
    ...
    procedure DrawNode(var rci: TRenderContextInfo; X, Y, Z: Single; Color:
      TGLColor);
  ...
  published
    ...
    property NodesAspect: TLineNodesAspect read FNodesAspect write SetNodesAspect
      default lnaAxes;
    {: Size for the various node aspects. }
    property NodeSize: Single read FNodeSize write SetNodeSize stored
      StoreNodeSize;
  end;
  ...
  public
    ...
    property NURBSTolerance: Single read FNURBSTolerance write
      SetNURBSTolerance;
...
begin
  inherited;
  defaultParams := (FMaxSize = 128) and (FMinSize = 0) and (FFadeTresholdSize =
    1);
...
begin
  ...
  FColor.Initialize((TGLLinesNodes(Collection).GetOwner as
    TGLLines).NodeColor.Color);
...
begin
  Result := not VectorEquals((TGLLinesNodes(Collection).GetOwner as
    TGLLines).NodeColor.Color,
    FColor.Color);
end;
...
procedure TGLQuadricObject.SetInvertedQuadricOrientation(quadric:
  PGLUquadricObj);
const
  cNormalDirectionToEnum: array[ndInside..ndOutside] of TGLEnum =
