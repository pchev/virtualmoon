...
{: VectorGeometry<p>
   ...
   all Intel processors after Pentium should be immune to this.<p>
	...
	<b>History : </b><font size=-1><ul>
      ...
      <li>20/11/12 - PW - Added CPP compatibility using records instead arrays for vectors,
...
{: Calculates norm of a vector which is defined as norm = v.V[0]*v.V[0] + ...<p>
   Also known as "Norm 2" in the math world, this is sqr(VectorLength). }
...
begin
   if VectorEquals(v1, v2) then begin
      ...
      q1:=QuaternionFromEuler(VectorGeometry.RadToDeg(v1.V[0]), VectorGeometry.RadToDeg(v1.V[1]), VectorGeometry.RadToDeg(v1.V[2]), eulZYX);
      q2:=QuaternionFromEuler(VectorGeometry.RadToDeg(v2.V[0]), VectorGeometry.RadToDeg(v2.V[1]), VectorGeometry.RadToDeg(v2.V[2]), eulZYX);
      qr:=QuaternionSlerp(q1, q2, t);
      m:=QuaternionToMatrix(qr);
      MatrixDecompose(m, tran);
      Result.V[0]:=tran[ttRotateX];
      Result.V[1]:=tran[ttRotateY];
      Result.V[2]:=tran[ttRotateZ];
   end;
end;
...
begin
   VectorGeometry.SinCos(alpha, s, c);
   Result.V[0]:=v.V[0];
   Result.V[1]:=c*v.V[1]+s*v.V[2];
   Result.V[2]:=c*v.V[2]-s*v.V[1];
end;
...
begin
   VectorGeometry.SinCos(alpha, s, c);
   Result.V[1]:=v.V[1];
   Result.V[0]:=c*v.V[0]+s*v.V[2];
   Result.V[2]:=c*v.V[2]-s*v.V[0];
end;
...
begin
   VectorGeometry.SinCos(alpha, s, c);
   vr.V[1]:=v.V[1];
   vr.V[0]:=c*v.V[0]+s*v.V[2];
   vr.V[2]:=c*v.V[2]-s*v.V[0];
end;
...
begin
   VectorGeometry.SinCos(alpha, s, c);
   Result.V[0]:=c*v.V[0]+s*v.V[1];
   Result.V[1]:=c*v.V[1]-s*v.V[0];
   Result.V[2]:=v.V[2];
end;
...
begin
  ...
  Tran[ttRotateY]:=VectorGeometry.ArcSin(-row0.V[Z]);
  if cos(Tran[ttRotateY]) <> 0 then begin
    Tran[ttRotateX]:=VectorGeometry.ArcTan2(row1.V[Z], row2.V[Z]);
    Tran[ttRotateZ]:=VectorGeometry.ArcTan2(row0.V[Y], row0.V[X]);
  end else begin
    tran[ttRotateX]:=VectorGeometry.ArcTan2(row1.V[X], row1.V[Y]);
    tran[ttRotateZ]:=0;
  end;
  // All done!
  Result:=True;
...
begin
   ...
   VectorGeometry.SinCos(t, w, p.V[1], p.V[0]);
end;
...
begin
  ...
  Result.V[0] := VectorGeometry.RadToDeg(-pitchangledif);
  Result.V[1] := VectorGeometry.RadToDeg(turnangledif);
end;
...
begin
  Result := VectorGeometry.ArcCos(VectorAngleCosine(
